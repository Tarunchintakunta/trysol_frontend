import React from 'react';
import { Card, Col, Row, Container } from 'react-bootstrap';
import 'bootstrap/dist/css/bootstrap.min.css';

const AdminDashboard = () => {
  return (
    <Container className="admin-dashboard" style={{ padding: '2rem' }}>
      {/* Logo Image */}
      <div style={{ display: 'flex', alignItems: 'center', marginBottom: '2rem' }}>
        <img
          src="data:image/jpeg;base64,"  // Placeholder link for logo
          alt="Logo"
          style={{ width: '60px', height: '60px', marginRight: '1rem' }}
        />
        <div style={{ display: 'table', justifyContent: 'center', alignItems: 'center', height: '10vh' }}>
          <br></br>
  <h2 style={{ fontWeight: 'bold', color: '#343a40' }}>Admin Dashboard</h2>
</div>

      </div>

      <Row className="g-4 mb-4">
        <Col md={6}>
          <Card style={{ border: 'none', boxShadow: '0 4px 8px rgba(0, 0, 0, 0.2)', backgroundColor: '#f8f9fa' }}>
            <Card.Body>
              <Card.Title style={{ color: '#007bff', fontWeight: '600' }}>View Employees</Card.Title>
              <Card.Text style={{ color: '#6c757d' }}>
                Access the list of all employees in the system.
              </Card.Text>
              <Card.Link href="/admin/view-employees" style={{ color: '#007bff', fontWeight: '500' }}>Go to View Employees</Card.Link>
            </Card.Body>
          </Card>
        </Col>
        
        <Col md={6}>
          <Card style={{ border: 'none', boxShadow: '0 4px 8px rgba(0, 0, 0, 0.2)', backgroundColor: '#e9f7ef' }}>
            <Card.Body>
              <Card.Title style={{ color: '#28a745', fontWeight: '600' }}>View Candidates</Card.Title>
              <Card.Text style={{ color: '#6c757d' }}>
                Access the list of all candidates in the system.
              </Card.Text>
              <Card.Link href="/admin/view-candidates" style={{ color: '#28a745', fontWeight: '500' }}>Go to View Candidates</Card.Link>
            </Card.Body>
          </Card>
        </Col>
      </Row>
      
      <Row className="g-4">
        <Col md={6}>
          <Card style={{ border: 'none', boxShadow: '0 4px 8px rgba(0, 0, 0, 0.2)', backgroundColor: '#ffebee' }}>
            <Card.Body>
              <Card.Title style={{ color: '#dc3545', fontWeight: '600' }}>Upload Candidates</Card.Title>
              <Card.Text style={{ color: '#6c757d' }}>
                Upload candidate profiles from an Excel file.
              </Card.Text>
              <Card.Link href="/admin/upload-candidates" style={{ color: '#dc3545', fontWeight: '500' }}>Go to Upload Candidates</Card.Link>
            </Card.Body>
          </Card>
        </Col>
        
        <Col md={6}>
          <Card style={{ border: 'none', boxShadow: '0 4px 8px rgba(0, 0, 0, 0.2)', backgroundColor: '#fff3cd' }}>
            <Card.Body>
              <Card.Title style={{ color: '#ffc107', fontWeight: '600' }}>Candidates Audit</Card.Title>
              <Card.Text style={{ color: '#6c757d' }}>
                Audit the uploaded candidate profiles.
              </Card.Text>
              <Card.Link href="/admin/candidates-audit" style={{ color: '#ffc107', fontWeight: '500' }}>Go to Candidates Audit</Card.Link>
            </Card.Body>
          </Card>
        </Col>
      </Row>
    </Container>
  );
};

export default AdminDashboard;
